...
export default function EmployeeDetailPage() {
  ...
  const { data } = useSWR(
    companyId ? ['employee-detail', params.id, companyId] : null,
    async ([, empId, cid]) => {
      const [{ data: employeeData }, { data: itemsData }] = await Promise.all([
        supabase.from('employees').select('*').eq('id', empId).eq('company_id', cid).maybeSingle(),
        supabase.from('commission_items').select('*').eq('company_id', cid).order('name'),
      ])
      ...
      if (employee.worker_type === 'commission') {
        const { data: ratesData } = await supabase
          .from('agent_item_rates').select('*, commission_items(id, name, default_rate)').eq('employee_id', empId)
        agentRates = (ratesData || []) as AgentItemRate[]
      }

      return {
        employee,
        commissionItems: (itemsData || []) as CommissionItem[],
        agentRates,
      }
    },
  ...
  const { employee, commissionItems, agentRates } = data
